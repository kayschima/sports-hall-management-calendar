    ...
    <div class="jumbotron">
        ...
        <form wire:submit.prevent="addUpdateSport">
            <div class="form-group">
                ...
                <div><p>@error('sportname')<small id="helpId" class="text-danger">{{ $message }}</small></p>@enderror</div>
                <button type="submit" class="btn btn-primary">{{__('Save')}}</button>
                <button type="button" class="btn btn-primary" wire:click="closeDialog">{{__('Cancel')}}</button>
            </div>
        </form>
    </div>
</div>
